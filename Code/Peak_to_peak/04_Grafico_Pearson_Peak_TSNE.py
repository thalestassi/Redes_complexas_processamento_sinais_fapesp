...
matrizes_a_peak = np.load(f"Matrizes_Adjacencia_Prominence/Matriz_autista_peak_prom_0_00.npy")
matrizes_c_peak = np.load(f"Matrizes_Adjacencia_Prominence/Matriz_controle_peak_prom_0_00.npy")

resultados_metricas_a_pearson = np.load("Metricas/Metricas_autistas_pearson_peak.npy")
resultados_metricas_c_pearson = np.load("Metricas/Metricas_controle_pearson_peak.npy")
...
indices_tsne2_cluster = np.where((tsne1 > 50) & (tsne2 > 0))[0]
...
print(a_indices_cluster)
print(c_indices_cluster)

matrizes_a_hilbert_tsne = np.zeros((len(a_indices_cluster), len(ondas), np.shape(x_a)[1], np.shape(x_a)[1]))
contador = 0
for i in range(np.shape(matrizes_a_peak)[0]):
    if i in a_indices_cluster:
        matrizes_a_hilbert_tsne[contador] = matrizes_a_peak[i]
        contador = contador + 1
...
for i in range(np.shape(matrizes_c_peak)[0]):
    if i in c_indices_cluster:
        matrizes_c_hilbert_tsne[contador] = matrizes_c_peak[i]
        contador = contador + 1
